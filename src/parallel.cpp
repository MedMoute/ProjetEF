...
void communication(VectorXd u, vector<vector<int> > voisins_partition, vector<vector<int> > voisins_interface)
{
    if (rang==0)
    {
        ...
        {
            valeurs_a_envoyer[i-1].resize(voisins_partition[i-1].size());
            valeurs_a_recevoir[i-1].resize(voisins_interface[i-1].size());
            for (unsigned int j=0;j<voisins_partition[i-1].size();j++)
            {
    ...
    else
    {
        ...
        valeurs_a_envoyer.resize(voisins_interface[rang-1].size());
        vector<double> valeurs_a_recevoir;
        valeurs_a_recevoir.resize(voisins_partition[rang-1].size());
